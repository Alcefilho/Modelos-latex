...
%---------------------------------------------------------------------
\graphicspath{
{./Figs/}                       % Caminho relativo
%{/home/salviano/Cursos/Figs/}  % Caminho completo
}
%---------------------------------------------------------------------
\usepackage{fancybox,amssymb}
\usepackage{pstricks}

\title{Criando apresentações no \LaTeX\ \\ com o pacote \texttt{Prosper}}
...
\maketitle
\begin{abstract}
  A classe \texttt{prosper} permite a produção de slides de alta qualidade;
  ela é facilmente extendível. Este documento tem por objetivo ser um manual
  para o usuário assim como uma nota técnica de como criar o seu próprio 
  estilo de slide. 
\end{abstract}

\section{Usando a classe}
%========================
Os arquivos \LaTeX\ usando a classe \texttt{prosper} podem 
eventualmente, após uma compilação, produzir como resultado 
dois diferentes formatos:
\begin{itemize}
\item o formato da Adobe\Registered\ \emph{PostScript}\tm\ para impressão 
  das transparências;
\item o formato da Adobe\Registered\ \emph{Portable Document Format} (PDF) para
  apresentar os slides em computadores com litores de PDF com suporte ao  
  Acrobat\Registered\ Reader no modo tela cheia.
\end{itemize}
